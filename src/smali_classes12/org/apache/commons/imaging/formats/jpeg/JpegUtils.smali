.class public Lorg/apache/commons/imaging/formats/jpeg/JpegUtils;
.super Lorg/apache/commons/imaging/common/BinaryFileParser;
.source "JpegUtils.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$Visitor;
    }
.end annotation


# direct methods
.method public constructor <init>()V
    .locals 1

    invoke-direct {p0}, Lorg/apache/commons/imaging/common/BinaryFileParser;-><init>()V

    sget-object v0, Ljava/nio/ByteOrder;->BIG_ENDIAN:Ljava/nio/ByteOrder;

    invoke-virtual {p0, v0}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils;->setByteOrder(Ljava/nio/ByteOrder;)V

    return-void
.end method

.method public static getMarkerName(I)Ljava/lang/String;
    .locals 1

    const v0, 0xffdd

    if-eq p0, v0, :cond_0

    packed-switch p0, :pswitch_data_0

    packed-switch p0, :pswitch_data_1

    packed-switch p0, :pswitch_data_2

    packed-switch p0, :pswitch_data_3

    const-string v0, "Unknown"

    return-object v0

    :pswitch_0
    const-string v0, "JPEG_APP15_MARKER"

    return-object v0

    :pswitch_1
    const-string v0, "JPEG_APP14_MARKER"

    return-object v0

    :pswitch_2
    const-string v0, "JPEG_APP13_MARKER"

    return-object v0

    :pswitch_3
    const-string v0, "JPEG_APP2_MARKER"

    return-object v0

    :pswitch_4
    const-string v0, "JPEG_APP1_MARKER"

    return-object v0

    :pswitch_5
    const-string v0, "JFIF_MARKER"

    return-object v0

    :pswitch_6
    const-string v0, "DQT_MARKER"

    return-object v0

    :pswitch_7
    const-string v0, "SOS_MARKER"

    return-object v0

    :pswitch_8
    const-string v0, "RST7_MARKER"

    return-object v0

    :pswitch_9
    const-string v0, "RST6_MARKER"

    return-object v0

    :pswitch_a
    const-string v0, "RST5_MARKER"

    return-object v0

    :pswitch_b
    const-string v0, "RST4_MARKER"

    return-object v0

    :pswitch_c
    const-string v0, "RST3_MARKER"

    return-object v0

    :pswitch_d
    const-string v0, "RST2_MARKER"

    return-object v0

    :pswitch_e
    const-string v0, "RST1_MARKER"

    return-object v0

    :pswitch_f
    const-string v0, "RST0_MARKER"

    return-object v0

    :pswitch_10
    const-string v0, "SOF15_MARKER"

    return-object v0

    :pswitch_11
    const-string v0, "SOF14_MARKER"

    return-object v0

    :pswitch_12
    const-string v0, "SOF13_MARKER"

    return-object v0

    :pswitch_13
    const-string v0, "DAC_MARKER"

    return-object v0

    :pswitch_14
    const-string v0, "SOF11_MARKER"

    return-object v0

    :pswitch_15
    const-string v0, "SOF10_MARKER"

    return-object v0

    :pswitch_16
    const-string v0, "SOF9_MARKER"

    return-object v0

    :pswitch_17
    const-string v0, "SOF8_MARKER"

    return-object v0

    :pswitch_18
    const-string v0, "SOF7_MARKER"

    return-object v0

    :pswitch_19
    const-string v0, "SOF6_MARKER"

    return-object v0

    :pswitch_1a
    const-string v0, "SOF5_MARKER"

    return-object v0

    :pswitch_1b
    const-string v0, "SOF4_MARKER"

    return-object v0

    :pswitch_1c
    const-string v0, "SOF3_MARKER"

    return-object v0

    :pswitch_1d
    const-string v0, "SOF2_MARKER"

    return-object v0

    :pswitch_1e
    const-string v0, "SOF1_MARKER"

    return-object v0

    :pswitch_1f
    const-string v0, "SOF0_MARKER"

    return-object v0

    :cond_0
    const-string v0, "DRI_MARKER"

    return-object v0

    nop

    :pswitch_data_0
    .packed-switch 0xffc0
        :pswitch_1f
        :pswitch_1e
        :pswitch_1d
        :pswitch_1c
        :pswitch_1b
        :pswitch_1a
        :pswitch_19
        :pswitch_18
        :pswitch_17
        :pswitch_16
        :pswitch_15
        :pswitch_14
        :pswitch_13
        :pswitch_12
        :pswitch_11
        :pswitch_10
        :pswitch_f
        :pswitch_e
        :pswitch_d
        :pswitch_c
        :pswitch_b
        :pswitch_a
        :pswitch_9
        :pswitch_8
    .end packed-switch

    :pswitch_data_1
    .packed-switch 0xffda
        :pswitch_7
        :pswitch_6
    .end packed-switch

    :pswitch_data_2
    .packed-switch 0xffe0
        :pswitch_5
        :pswitch_4
        :pswitch_3
    .end packed-switch

    :pswitch_data_3
    .packed-switch 0xffed
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method


# virtual methods
.method public dumpJFIF(Lorg/apache/commons/imaging/common/bytesource/ByteSource;)V
    .locals 1
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lorg/apache/commons/imaging/ImageReadException;,
            Ljava/io/IOException;
        }
    .end annotation

    new-instance v0, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$1;

    invoke-direct {v0, p0}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$1;-><init>(Lorg/apache/commons/imaging/formats/jpeg/JpegUtils;)V

    invoke-virtual {p0, p1, v0}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils;->traverseJFIF(Lorg/apache/commons/imaging/common/bytesource/ByteSource;Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$Visitor;)V

    return-void
.end method

.method public traverseJFIF(Lorg/apache/commons/imaging/common/bytesource/ByteSource;Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$Visitor;)V
    .locals 14
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lorg/apache/commons/imaging/ImageReadException;,
            Ljava/io/IOException;
        }
    .end annotation

    const-string v0, "segmentLengthBytes"

    invoke-virtual {p1}, Lorg/apache/commons/imaging/common/bytesource/ByteSource;->getInputStream()Ljava/io/InputStream;

    move-result-object v1

    :try_start_0
    sget-object v2, Lorg/apache/commons/imaging/formats/jpeg/JpegConstants;->SOI:Lorg/apache/commons/imaging/common/BinaryConstant;

    const-string v3, "Not a Valid JPEG File: doesn\'t begin with 0xffd8"

    invoke-static {v1, v2, v3}, Lorg/apache/commons/imaging/common/BinaryFunctions;->readAndVerifyBytes(Ljava/io/InputStream;Lorg/apache/commons/imaging/common/BinaryConstant;Ljava/lang/String;)V

    const/4 v2, 0x0

    :goto_0
    const/4 v3, 0x2

    new-array v4, v3, [B

    :goto_1
    const/4 v5, 0x1

    aget-byte v6, v4, v5

    const/4 v7, 0x0

    aput-byte v6, v4, v7

    const-string v6, "marker"

    const-string v8, "Could not read marker"

    invoke-static {v6, v1, v8}, Lorg/apache/commons/imaging/common/BinaryFunctions;->readByte(Ljava/lang/String;Ljava/io/InputStream;Ljava/lang/String;)B

    move-result v6

    aput-byte v6, v4, v5

    aget-byte v6, v4, v7

    const/16 v8, 0xff

    and-int/2addr v6, v8

    if-ne v6, v8, :cond_8

    aget-byte v6, v4, v5

    and-int/2addr v6, v8

    if-eq v6, v8, :cond_8

    aget-byte v6, v4, v7

    and-int/2addr v6, v8

    shl-int/lit8 v6, v6, 0x8

    aget-byte v5, v4, v5

    and-int/2addr v5, v8

    or-int v11, v6, v5

    const v5, 0xffd9

    if-eq v11, v5, :cond_4

    const v5, 0xffda

    if-ne v11, v5, :cond_0

    goto :goto_2

    :cond_0
    invoke-static {v0, v1, v3, v0}, Lorg/apache/commons/imaging/common/BinaryFunctions;->readBytes(Ljava/lang/String;Ljava/io/InputStream;ILjava/lang/String;)[B

    move-result-object v5

    move-object v12, v5

    invoke-virtual {p0}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils;->getByteOrder()Ljava/nio/ByteOrder;

    move-result-object v5

    invoke-static {v12, v5}, Lorg/apache/commons/imaging/common/ByteConversions;->toUInt16([BLjava/nio/ByteOrder;)I

    move-result v5

    move v13, v5

    if-lt v13, v3, :cond_3

    const-string v3, "Segment Data"

    add-int/lit8 v5, v13, -0x2

    const-string v6, "Invalid Segment: insufficient data"

    invoke-static {v3, v1, v5, v6}, Lorg/apache/commons/imaging/common/BinaryFunctions;->readBytes(Ljava/lang/String;Ljava/io/InputStream;ILjava/lang/String;)[B

    move-result-object v10

    move-object/from16 v5, p2

    move v6, v11

    move-object v7, v4

    move v8, v13

    move-object v9, v12

    invoke-interface/range {v5 .. v10}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$Visitor;->visitSegment(I[BI[B[B)Z

    move-result v3
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_1

    if-nez v3, :cond_2

    if-eqz v1, :cond_1

    invoke-virtual {v1}, Ljava/io/InputStream;->close()V

    :cond_1
    return-void

    :cond_2
    add-int/lit8 v2, v2, 0x1

    goto :goto_0

    :cond_3
    :try_start_1
    new-instance v0, Lorg/apache/commons/imaging/ImageReadException;

    const-string v3, "Invalid segment size"

    invoke-direct {v0, v3}, Lorg/apache/commons/imaging/ImageReadException;-><init>(Ljava/lang/String;)V

    throw v0

    :cond_4
    :goto_2
    invoke-interface/range {p2 .. p2}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$Visitor;->beginSOS()Z

    move-result v0
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_1

    if-nez v0, :cond_6

    if-eqz v1, :cond_5

    invoke-virtual {v1}, Ljava/io/InputStream;->close()V

    :cond_5
    return-void

    :cond_6
    :try_start_2
    invoke-static {v1}, Lorg/apache/commons/imaging/common/BinaryFunctions;->getStreamBytes(Ljava/io/InputStream;)[B

    move-result-object v0
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_1

    move-object/from16 v5, p2

    :try_start_3
    invoke-interface {v5, v11, v4, v0}, Lorg/apache/commons/imaging/formats/jpeg/JpegUtils$Visitor;->visitSOS(I[B[B)V

    nop

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {v0, v2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v3, " markers"

    invoke-virtual {v0, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Lorg/apache/commons/imaging/internal/Debug;->debug(Ljava/lang/String;)V
    :try_end_3
    .catchall {:try_start_3 .. :try_end_3} :catchall_0

    if-eqz v1, :cond_7

    invoke-virtual {v1}, Ljava/io/InputStream;->close()V

    :cond_7
    return-void

    :catchall_0
    move-exception v0

    goto :goto_3

    :cond_8
    move-object/from16 v5, p2

    goto/16 :goto_1

    :catchall_1
    move-exception v0

    move-object/from16 v5, p2

    :goto_3
    move-object v2, v0

    if-eqz v1, :cond_9

    :try_start_4
    invoke-virtual {v1}, Ljava/io/InputStream;->close()V
    :try_end_4
    .catchall {:try_start_4 .. :try_end_4} :catchall_2

    goto :goto_4

    :catchall_2
    move-exception v0

    move-object v3, v0

    invoke-virtual {v2, v3}, Ljava/lang/Throwable;->addSuppressed(Ljava/lang/Throwable;)V

    :cond_9
    :goto_4
    throw v2
.end method
