.class final Lln/q$d;
.super Lln/p;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lln/q;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x8
    name = null
.end annotation


# direct methods
.method constructor <init>(Lln/b1;)V
    .locals 0

    invoke-direct {p0, p1}, Lln/p;-><init>(Lln/b1;)V

    return-void
.end method

.method private static synthetic e(I)V
    .locals 5

    const/4 v0, 0x3

    new-array v0, v0, [Ljava/lang/Object;

    const/4 v1, 0x0

    const/4 v2, 0x1

    const/4 v3, 0x2

    if-eq p0, v2, :cond_1

    if-eq p0, v3, :cond_0

    const-string v4, "descriptor"

    aput-object v4, v0, v1

    goto :goto_0

    :cond_0
    const-string v4, "from"

    aput-object v4, v0, v1

    goto :goto_0

    :cond_1
    const-string/jumbo v4, "what"

    aput-object v4, v0, v1

    :goto_0
    const-string v1, "kotlin/reflect/jvm/internal/impl/descriptors/DescriptorVisibilities$1"

    aput-object v1, v0, v2

    if-eq p0, v2, :cond_2

    if-eq p0, v3, :cond_2

    const-string p0, "hasContainingSourceFile"

    aput-object p0, v0, v3

    goto :goto_1

    :cond_2
    const-string p0, "isVisible"

    aput-object p0, v0, v3

    :goto_1
    const-string p0, "Argument for @NotNull parameter \'%s\' of %s.%s must not be null"

    invoke-static {p0, v0}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object p0

    new-instance v0, Ljava/lang/IllegalArgumentException;

    invoke-direct {v0, p0}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0
.end method


# virtual methods
.method public final c(Lqo/d;Lln/o;Lln/k;)Z
    .locals 3

    if-eqz p3, :cond_a

    invoke-static {p2}, Lio/g;->F(Lln/k;)Z

    move-result p1

    const/4 v0, 0x1

    const/4 v1, 0x0

    if-eqz p1, :cond_1

    invoke-static {p3}, Lio/g;->i(Lln/k;)Lln/r0;

    move-result-object p1

    sget-object v2, Lln/r0;->a:Lln/r0;

    if-eq p1, v2, :cond_0

    const/4 p1, 0x1

    goto :goto_0

    :cond_0
    const/4 p1, 0x0

    :goto_0
    if-eqz p1, :cond_1

    invoke-static {p2, p3}, Lln/q;->f(Lln/k;Lln/k;)Z

    move-result p1

    return p1

    :cond_1
    instance-of p1, p2, Lln/j;

    if-eqz p1, :cond_2

    move-object p1, p2

    check-cast p1, Lln/j;

    invoke-interface {p1}, Lln/j;->b()Lln/i;

    move-result-object p1

    invoke-static {p1}, Lio/g;->C(Lln/k;)Z

    move-result v2

    if-eqz v2, :cond_2

    invoke-static {p1}, Lio/g;->F(Lln/k;)Z

    move-result p1

    if-eqz p1, :cond_2

    instance-of p1, p3, Lln/j;

    if-eqz p1, :cond_2

    invoke-interface {p3}, Lln/k;->b()Lln/k;

    move-result-object p1

    invoke-static {p1}, Lio/g;->F(Lln/k;)Z

    move-result p1

    if-eqz p1, :cond_2

    invoke-static {p2, p3}, Lln/q;->f(Lln/k;Lln/k;)Z

    move-result p1

    if-eqz p1, :cond_2

    return v0

    :cond_2
    if-eqz p2, :cond_4

    invoke-interface {p2}, Lln/k;->b()Lln/k;

    move-result-object p2

    instance-of p1, p2, Lln/e;

    if-eqz p1, :cond_3

    invoke-static {p2}, Lio/g;->u(Lln/k;)Z

    move-result p1

    if-eqz p1, :cond_4

    :cond_3
    instance-of p1, p2, Lln/b0;

    if-eqz p1, :cond_2

    :cond_4
    if-nez p2, :cond_5

    return v1

    :cond_5
    :goto_1
    if-eqz p3, :cond_9

    if-ne p2, p3, :cond_6

    return v0

    :cond_6
    instance-of p1, p3, Lln/b0;

    if-eqz p1, :cond_8

    instance-of p1, p2, Lln/b0;

    if-eqz p1, :cond_7

    move-object p1, p2

    check-cast p1, Lln/b0;

    invoke-interface {p1}, Lln/b0;->d()Lho/c;

    move-result-object p1

    move-object v2, p3

    check-cast v2, Lln/b0;

    invoke-interface {v2}, Lln/b0;->d()Lho/c;

    move-result-object v2

    invoke-virtual {p1, v2}, Lho/c;->equals(Ljava/lang/Object;)Z

    move-result p1

    if-eqz p1, :cond_7

    invoke-static {p3, p2}, Lio/g;->b(Lln/k;Lln/k;)Z

    move-result p1

    if-eqz p1, :cond_7

    goto :goto_2

    :cond_7
    const/4 v0, 0x0

    :goto_2
    return v0

    :cond_8
    invoke-interface {p3}, Lln/k;->b()Lln/k;

    move-result-object p3

    goto :goto_1

    :cond_9
    return v1

    :cond_a
    const/4 p1, 0x2

    invoke-static {p1}, Lln/q$d;->e(I)V

    const/4 p1, 0x0

    throw p1
.end method
